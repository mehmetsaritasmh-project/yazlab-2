...
1.	İstek Yönlendirme (Routing): İstekleri ilgili mikroservise iletmek.
2.	Güvenlik ve Yetkilendirme: Merkezi bir kimlik doğrulama noktası oluşturmak.
3.	Loglama: Tüm trafiği kayıt altına alarak performans analizi yapmak.
...
1.	Giriş ve Loglama: Her istek Dispatcher tarafından karşılanır ve MongoDB'ye loglanır.
2.	Yetki Kontrolü: İsteğin halka açık (public) mı yoksa korumalı mı olduğu sorgulanır.
3.	Güvenlik Doğrulaması: Korumalı kaynaklar için Token geçerliliği sorgulanır.
4.	Yönlendirme: Doğrulanmış istekler hedef mikroservise iletilir.
...
graph TD A[İstemci İstek Gönderir] --> B{Dispatcher İsteği Karşılar}
...
--> I[İsteği Mikroservise İlet] 
I --> J[Yanıtı İstemciye Dön]
```


Şema Açıklaması:
Bu akış diyagramı (flowchart), sistemin güvenlik ve yönlendirme mantığını adım adım özetlemektedir:
1.	Merkezi Karşılama: İstemciden gelen tüm HTTP istekleri ilk olarak Dispatcher servisi tarafından karşılanır.
2.	İlk Güvenlik Kontrolü: Dispatcher, isteğin içinde bir kimlik doğrulama anahtarı (Auth Token) olup olmadığını denetler. Eğer token yoksa, işlem doğrudan 401 Yetkisiz Hatası ile sonlandırılır.
3.	Doğrulama Süreci: Token mevcutsa, bu bilgi Auth Service birimine gönderilir. Servis, token'ın süresinin dolup dolmadığını veya geçerli olup olmadığını kontrol eder. Geçersiz durumlarda 403 Yasaklı/Geçersiz yanıtı döner.
4.	Akıllı Yönlendirme: Doğrulamadan geçen istekler, URL yapısına göre ilgili mikroservise (User veya Product) yönlendirilir.
...
Şema Açıklaması: 
* Merkezi Giriş: İstemciden gelen tüm istekler Dispatcher Service (Port 8000) üzerinden sisteme dahil olur. 
...
3.3.3
Dispatcher Yönlendirme Algoritması
...
•	Güvenlik Doğrulaması: Hedef servis "kamu açık" değilse, Header kısmındaki Authorization token'ı Auth Service üzerinden kontrol edilir.
•	İstek İletimi : Doğrulanan istek, hedef servisin iç ağdaki adresine  aynen iletilir.
•	Yanıtın Dönülmesi: Alt servisten gelen cevap, hiçbir değişikliğe uğramadan istemciye geri gönderilir.
Yalancı Kod (Pseudo-Code) Görünümü:
...
•	5. Uygulamaya Ait Açıklamalar ve Test Süreçlerikanıtlanmıştır.
•	Birim (Unit) Test Senaryoları ve Sonuçları

•	Birim (Unit) Testleri: pytest kullanılarak test_dispatcher.py üzerinden yönlendirme kabiliyetleri ölçülür.
•	BTest Aracı: Projede birim testler için pytest kütüphanesi kullanılmıştır.
•	Senaryo 1 (Doğru Yönlendirme): /users endpoint'ine gönderilen geçerli bir 
•	isteğin user_service birimine başarıyla yönlendirildiği ve 200 OK yanıtı alındığı doğrulanmıştır.
...
•	Sonuç: Hazırlanan 15 farklı test senaryosunun tamamı başarıyla (PASSED) sonuçlanmış, sistemin yönlendirme ve güvenlik mantığının hatasız çalıştığı 

•	Yük (Load) Testleri: Locust kullanılarak yüzlerce sanal kullanıcının sisteme etkisi simüle edilir (locustfil e.py).
![WhatsApp Image 2026-04-02 at 16 37 06](https://github.com/user-attachments/assets/527b72c2-1af3-4ba2-964d-fd1833a37c35)

Yük (Load) Testi Senaryosu ve Performans Verileri
•	Test Aracı: Sistemin yüksek trafik altındaki davranışı Locust aracı ile simüle edilmiştir.
...
•	Yanıt Süresi: Normal yük altında 10-15ms olan yanıt süreleri, yoğun stres altında ortalama 45-60ms seviyelerinde kalmıştır.
...
![WhatsApp Image 2026-04-02 at 16 37 06 (1)](https://github.com/user-attachments/assets/63718f1a-a3f1-46d2-8e63-cc3d4c490439)


Metrik İzleme (Monitoring) Çıktıları
Elinizdeki Prometheus ve Grafana görsellerini bu başlık altında sunmalısınız:
•	Sistem Sağlığı: Prometheus arayüzünde (Bkz: Şekil Y) dispatcher_service:8000/metrics endpoint'inin aktif (UP) olduğu ve metriklerin başarıyla kazındığı (scrape) görülmektedir.
